-- Fix get_monthly_reports function to match frontend expected fields
  ...
  package_stats AS (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE p.status IN (
        'pending_purchase', 'payment_confirmed', 'purchased', 'in_transit',
        'received_by_traveler', 'pending_office_confirmation', 'delivered_to_office', 'completed'
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'average_package_value', CASE 
        WHEN md.total_packages > 0 
        THEN ROUND(md.total_revenue / md.total_packages, 2)
        ELSE 0 
      END,
