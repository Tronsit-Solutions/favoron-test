...
-- Fix: packages where no traveler submitted a quote should return to 'approved', not 'quote_expired'
-- Also fix the Puma Softride Escalate package (8532da77) that was incorrectly marked quote_expired
...
BEGIN
  ...
  -- If so, decide: quote_expired (shopper ignored quotes) vs approved (no quotes received)
  FOR package_record IN
  ...
  LOOP
    ...
    IF remaining_active = 0 THEN
      -- Check if any traveler actually submitted a quote before expiring
      SELECT COUNT(*) INTO submitted_quote_count
      FROM public.package_assignments pa
      WHERE pa.package_id = package_record.id
        AND pa.status = 'bid_expired'
        AND pa.quote IS NOT NULL;

      IF submitted_quote_count > 0 THEN
        -- At least one traveler submitted a quote but shopper didn't respond → quote_expired
        UPDATE public.packages
        ...
        END LOOP;
      ELSE
        -- No traveler ever submitted a quote → back to approved for reassignment
        UPDATE public.packages
        SET status = 'approved',
            matched_trip_id = NULL,
            matched_assignment_expires_at = NULL,
        ...
        LOOP
          PERFORM public.create_notification(
            admin_user_id,
            '📦 Paquete disponible para reasignación',
            CONCAT('Ningún viajero cotizó "', package_record.item_description, '". El paquete regresó a Solicitudes para reasignación.'),
            'package',
            'high',
            NULL,
            jsonb_build_object(
              'package_id', package_record.id,
              'action', 'no_quotes_received_reassign'
            )
          );
        END LOOP;
      END IF;
...
-- Fix: Revert Puma Softride Escalate package that was incorrectly marked quote_expired
-- (no traveler ever submitted a quote)
UPDATE public.packages
SET status = 'approved',
    matched_trip_id = NULL,
    matched_assignment_expires_at = NULL,
    updated_at = NOW()
WHERE id = '8532da77-78cb-4fe5-af09-821ca4343472'
  AND status = 'quote_expired';
