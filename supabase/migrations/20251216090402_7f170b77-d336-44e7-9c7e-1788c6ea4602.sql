-- Drop and recreate create_payment_order_with_snapshot with updated status list
...
BEGIN
  -- Capture current package data snapshot with ALL delivery states
  SELECT jsonb_agg(
    jsonb_build_object(
      'package_id', p.id,
  ...
  -- Create the payment order with the snapshot
  ...
  VALUES (
    ...
    _bank_account_type,
    COALESCE(_historical_packages, '[]'::jsonb),
    'pending'
  )
  RETURNING id INTO _payment_order_id;

  -- Update trip_payment_accumulator to mark payment order as created
