-- Fix the trigger issue by dropping it with CASCADE first
-- Then consolidate phone numbers into single field for WhatsApp compatibility

-- Drop the problematic trigger and function that references non-existent banking fields
DROP FUNCTION IF EXISTS public.validate_profile_banking_update() CASCADE;
