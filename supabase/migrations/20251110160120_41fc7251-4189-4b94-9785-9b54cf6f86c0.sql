-- Update create_notification_with_direct_email to support WhatsApp notifications
...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  -- Send WhatsApp if conditions are met
  IF should_send_whatsapp THEN
    BEGIN
      RAISE NOTICE '📱 Sending WhatsApp notification to: %', user_phone;
      
      SELECT INTO http_result * FROM extensions.http((
        'POST',
        'https://dfhoduirmqbarjnspbdh.supabase.co/functions/v1/send-whatsapp-notification',
        ARRAY[
          extensions.http_header('Content-Type', 'application/json'),
      ...
      RAISE NOTICE '📱 WhatsApp API response status: %, content: %', http_result.status, http_result.content;
      
      IF http_result.status BETWEEN 200 AND 299 THEN
        RAISE NOTICE '✅ WhatsApp sent successfully for user % (notification: %)', _user_id, notification_id;
      ELSE
        RAISE WARNING '❌ Failed to send WhatsApp for user % (notification: %): HTTP status %', _user_id, notification_id, http_result.status;
      END IF;
      
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING '💥 Exception while sending WhatsApp for user % (notification: %): %', _user_id, notification_id, SQLERRM;
    END;
  END IF;
