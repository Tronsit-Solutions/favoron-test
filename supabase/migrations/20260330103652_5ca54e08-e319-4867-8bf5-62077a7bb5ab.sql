-- Modify expire_unresponded_assignments: packages go to quote_expired instead of approved
-- when all assignments expire
...
-- Data fix: Expire all current bid_submitted zombies immediately
UPDATE public.package_assignments
SET status = 'bid_expired', expires_at = NULL, updated_at = NOW()
WHERE status = 'bid_submitted';

-- Move parent packages to quote_expired if no active assignments remain
UPDATE public.packages
SET status = 'quote_expired', matched_trip_id = NULL, updated_at = NOW()
WHERE id IN (
  SELECT DISTINCT pa.package_id FROM public.package_assignments pa
  WHERE pa.status = 'bid_expired'
)
AND NOT EXISTS (
  SELECT 1 FROM public.package_assignments pa2
