-- Secure trips_with_user view using has_role() and security_barrier, and restrict privileges
...
COMMENT ON VIEW public.trips_with_user IS
  'Secure view: owner or admin can see rows. Includes profile data (phone/email). Backed by RLS on trips and profiles.';

-- Ensure no public/anon access and only authenticated can SELECT
