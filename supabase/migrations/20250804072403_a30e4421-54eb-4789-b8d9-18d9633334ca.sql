-- Mejorar el trigger de notificaciones para viajeros
-- Agregar notificación cuando paquete pasa a pending_purchase (pago completado)
...
BEGIN
  ...
  IF traveler_id IS NOT NULL THEN
    ...
    -- 3. NUEVO: Cuando el paquete pasa a pending_purchase (pago aprobado por admin)
    ELSIF (OLD.status IS NULL OR OLD.status != 'pending_purchase') AND NEW.status = 'pending_purchase' THEN
      notification_title := '✅ Pago aprobado - Procede con la compra';
      notification_message := CONCAT('¡Perfecto! El pago del shopper para "', NEW.item_description, '" ha sido aprobado por Favorón. Ya puedes proceder a comprar el artículo según las instrucciones.');
      notification_priority := 'high';
      
      RAISE NOTICE '🔍 CASE 2.5 - Payment approved (pending_purchase) detected';
