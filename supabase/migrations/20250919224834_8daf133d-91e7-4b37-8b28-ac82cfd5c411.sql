-- Fix the trigger issue by dropping the banking validation trigger first
-- Then consolidate phone numbers into single field for WhatsApp compatibility

-- Drop the problematic trigger that references non-existent banking fields
DROP TRIGGER IF EXISTS validate_banking_trigger ON public.profiles;
DROP FUNCTION IF EXISTS public.validate_profile_banking_update();

-- Now consolidate phone numbers
