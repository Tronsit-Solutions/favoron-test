-- Modificar función notify_traveler_package_status para agregar más logging y corregir la lógica
...
BEGIN
  ...
  IF traveler_id IS NOT NULL THEN
    ...
    IF (OLD.matched_trip_id IS NULL OR OLD.matched_trip_id = '') AND NEW.matched_trip_id IS NOT NULL THEN
    ...
    IF notification_title IS NOT NULL AND notification_message IS NOT NULL THEN
      RAISE NOTICE '🔍 Creating notification: Title=%, Message=%', notification_title, notification_message;
