-- Fix get_monthly_reports function to resolve variable scope issues
...
BEGIN
  ...
  WITH date_bounds AS (
    SELECT 
      ...
      COALESCE(
        CASE 
          WHEN end_date IS NOT NULL THEN DATE_TRUNC('month', end_date + INTERVAL '1 month')::date
          ELSE GREATEST(
  ...
  monthly_data AS (
    ...
    FROM date_bounds db
    CROSS JOIN generate_series(db.effective_start_date::timestamp, db.effective_end_date::timestamp, '1 month'::interval) d(month)
    WHERE d.month < db.effective_end_date::timestamp
  ),
  package_stats AS (
    SELECT 
    ...
    FROM packages, date_bounds db
    WHERE packages.created_at >= db.effective_start_date::timestamp 
      AND packages.created_at < db.effective_end_date::timestamp
  ...
  package_status_counts AS (
    ...
    FROM (
      ...
      FROM packages, date_bounds db
      WHERE created_at >= db.effective_start_date::timestamp 
        AND created_at < db.effective_end_date::timestamp
  ...
  trip_stats AS (
    ...
    FROM trips, date_bounds db
    WHERE trips.created_at >= db.effective_start_date::timestamp 
      AND trips.created_at < db.effective_end_date::timestamp
