...
BEGIN
  ...
  -- Log admin access to payment orders
  IF is_admin AND TG_OP IN ('SELECT', 'UPDATE', 'DELETE') THEN
...
-- Apply audit trigger to payment_orders table
CREATE TRIGGER audit_payment_order_access
  AFTER SELECT OR UPDATE OR DELETE ON public.payment_orders
  FOR EACH ROW
...
'Audit trigger that logs all admin access to payment orders for security monitoring.';
