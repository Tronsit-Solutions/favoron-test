-- Step 1: Add unique partial index to prevent multiple active payment orders per trip
CREATE UNIQUE INDEX CONCURRENTLY idx_payment_orders_unique_active_per_trip 
ON payment_orders (traveler_id, trip_id) 
WHERE status IN ('pending', 'completed');

-- Step 2: Make the create_payment_order_with_snapshot function idempotent
...
-- Step 3: Clean up Daniela's duplicate orders (mark older one as cancelled)
...
WHERE id = (
  ...
  WHERE traveler_id = '5e3c944e-9130-4ea7-8165-b8ec9d5abf6f' 
    AND trip_id = '01cafd38-85e6-468b-bca6-2df0604097dd'
    AND status = 'pending'
  ORDER BY created_at ASC 
  LIMIT 1
);

-- Step 4: Audit and clean up any other duplicate orders
