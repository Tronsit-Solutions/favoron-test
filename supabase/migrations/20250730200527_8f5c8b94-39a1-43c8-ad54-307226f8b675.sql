-- Fix the get_monthly_reports function with correct column names
      ...
      COALESCE(SUM(
        CASE 
          WHEN p.status IN ('delivered_to_office', 'received_by_traveler') 
            ...
            AND (p.quote->>'totalPrice') IS NOT NULL
      ...
      COALESCE(SUM(
        CASE 
          WHEN p.status IN ('delivered_to_office', 'received_by_traveler') 
            AND p.quote IS NOT NULL 
            AND (p.quote->>'price') IS NOT NULL
            AND (p.quote->>'serviceFee') IS NOT NULL
            AND (p.quote->>'price')::DECIMAL > 0
