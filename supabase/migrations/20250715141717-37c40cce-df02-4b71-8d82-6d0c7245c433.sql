-- Fix the get_monthly_reports function by correcting the table alias issue
...
BEGIN
  ...
  package_stats AS (
    SELECT 
      ...
      SUM(CASE WHEN packages.status IN ('delivered_to_office', 'received_by_traveler') THEN 1 ELSE 0 END) as completed_deliveries,
      SUM(CASE WHEN packages.status NOT IN ('delivered_to_office', 'received_by_traveler', 'rejected') THEN 1 ELSE 0 END) as pending_requests,
      
      -- Métricas financieras
      COALESCE(SUM(
        CASE 
          WHEN packages.status IN ('delivered_to_office', 'received_by_traveler') 
    ...
    WHERE packages.created_at >= start_date 
      AND packages.created_at < end_date
  ...
  package_status_counts AS (
    ...
    GROUP BY packages.period_start
  ),
  trip_stats AS (
    SELECT 
    ...
    WHERE trips.created_at >= start_date 
      AND trips.created_at < end_date
