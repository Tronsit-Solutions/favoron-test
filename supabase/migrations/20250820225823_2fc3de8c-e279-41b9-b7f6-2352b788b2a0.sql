-- Add historical_packages field to payment_orders table for traceability
ALTER TABLE public.payment_orders 
ADD COLUMN historical_packages JSONB DEFAULT NULL;

-- Update the notify_admins_payment_order function to capture package snapshot
CREATE OR REPLACE FUNCTION public.create_payment_order_with_snapshot(_traveler_id uuid, _trip_id uuid, _amount numeric, _bank_name text, _bank_account_holder text, _bank_account_number text, _bank_account_type text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- Capture packages snapshot for this trip
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', p.id,
