-- Drop and recreate trips_with_user view to make it secure
-- The view should only be accessible through controlled functions

-- Drop the existing view
DROP VIEW IF EXISTS public.trips_with_user;

-- Drop and recreate the RPC function with proper security
DROP FUNCTION IF EXISTS public.get_admin_trips_with_user();

-- Create a secure function that returns trip data with user info (admin only)
...
-- Grant execute permission only to authenticated users (RLS will enforce admin check)
GRANT EXECUTE ON FUNCTION public.get_admin_trips_with_user() TO authenticated;

-- Revoke any existing permissions on the old view name (cleanup)
