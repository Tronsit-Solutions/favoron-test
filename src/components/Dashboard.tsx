    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="w-full mobile-container py-2 sm:py-6 lg:py-8 max-w-full box-border">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-3 sm:space-y-6 min-w-0 w-full max-w-full">
          ...
          <TabsContent value="packages" className="space-y-4 sm:space-y-6 min-w-0 w-full max-w-full overflow-visible pr-4 sm:pr-0">
            ...
            {userPackages.filter(pkg => {
              ...
              <div className="grid gap-3 sm:gap-4 md:gap-6 w-full max-w-full min-w-0 box-border">
                 {userPackages.filter(pkg => {
                   // Excluir paquetes archivados y cancelados del dashboard activo
                    if (pkg.status === 'archived_by_shopper' || pkg.status === 'cancelled') return false;
                    // Ocultar completados solo si ya dieron feedback o lo omitieron
                 ...
                 .map((pkg) => (
                      <CollapsiblePackageCard
                        ...
                        // Multi-assignment props
                         ...
                         onRejectAllQuotes={handleRejectAllQuotes}
                      />
                ))}
              </div>
            )}
