...
import { Calendar, MapPin, Package, DollarSign, CheckCircle, ShoppingBag, ChevronDown, User, Receipt } from "lucide-react";
import { useState, useEffect } from "react";
import { useRefundOrders, RefundOrder } from "@/hooks/useRefundOrders";
...
const PackageHistory = ({ packages, trips }: PackageHistoryProps) => {
  ...
  const [packageRefunds, setPackageRefunds] = useState<Record<string, RefundOrder | null>>({});
  const [selectedReceipt, setSelectedReceipt] = useState<{url: string, filename: string} | null>(null);
  const { getRefundOrdersForPackage } = useRefundOrders();

  const toggleExpanded = (id: string) => {
    setExpandedIds(prev => ({ ...prev, [id]: !prev[id] }));
  };

  // Cargar reembolsos para paquetes cancelados
  useEffect(() => {
    const loadRefunds = async () => {
      const cancelledPackages = completedPackages.filter(p => p.status === 'cancelled');
      if (cancelledPackages.length === 0) return;

      const refundPromises = cancelledPackages.map(async (pkg) => {
        const refunds = await getRefundOrdersForPackage(pkg.id);
        const refundWithReceipt = refunds.find(r => r.receipt_url) || refunds[0];
        return { packageId: pkg.id, refund: refundWithReceipt || null };
      });

      const results = await Promise.all(refundPromises);
      const refundMap: Record<string, RefundOrder | null> = {};
      results.forEach(r => { refundMap[r.packageId] = r.refund; });
      setPackageRefunds(refundMap);
    };
  ...
  return (
    <>
                      ...
                      <div className="flex flex-col space-y-3 sm:flex-row sm:justify-between sm:items-center sm:space-y-0">
                        ...
                        <div className="flex flex-col space-y-2 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-3">
                          <div className="flex items-center space-x-2">
                            ...
                            {/* Icono de comprobante de reembolso */}
                            {pkg.status === 'cancelled' && packageRefunds[pkg.id]?.receipt_url && (
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        const refund = packageRefunds[pkg.id];
                                        if (refund?.receipt_url) {
                                          setSelectedReceipt({
                                            url: refund.receipt_url,
                                            filename: refund.receipt_filename || 'comprobante-reembolso'
                                          });
                                        }
                                      }}
                                    >
                                  ...
                                  <TooltipContent>
                                    <p>Ver comprobante de reembolso</p>
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            )}
      ...
      {/* Modal para ver comprobante de reembolso */}
      <ReceiptViewerModal
