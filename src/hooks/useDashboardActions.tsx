...
export const useDashboardActions = (
  ...
  const handleTripSubmit = async (tripData: any) => {
    ...
    try {
      ...
      // Notify about boost code being saved (will be applied when accumulator is created)
      if (tripData.boostCode && createdTrip?.id) {
        toast({
          title: "🚀 Código de boost guardado",
          description: "El boost se aplicará automáticamente cuando se procesen tus entregas.",
        });
      }
  ...
  const handleMatchPackage = async (packageId: string, tripId: string, adminTip?: number, productsWithTips?: any[], allTripIds?: string[]) => {
    if (!updatePackage) {
      throw new Error('updatePackage function not available');
    }
    ...
    if (!adminTip || adminTip <= 0) {
      ...
      throw new Error('Admin tip is required');
    }
